  ...
  render() {
    return (
      <div className='pv2'>
        ...
        {this.props.schedules[0].stopRouteDirectionSchedules.map(rds => (
          (<div className="pa2" style={{background: '#eee', margin: '.5em'}}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', flexWrap: 'wrap', maxHeight: rds.scheduleStopTimes.length > 80 ? 350 : 150, alignContent: 'flex-start', textAlign: 'left' }}>
            {rds.scheduleStopTimes.map(sst => (
              <span className="ph2" style={{fontSize: '.8em'}}>{moment(sst.arrivalTime).format('h:mma')}</span>
            ))}
          </div>
          </div>)
        ))}
