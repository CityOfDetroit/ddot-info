import sqlalchemy
import json
engine = sqlalchemy.create_engine('postgresql://gisteam@localhost/ddotinfo')
conn = engine.connect()
...
if __name__ == "__main__":
  ...
  def get_best_xfer(origin_stop='7921', route='49', direction='eastbound'):
    ...
    query = """
        ...
        inner join gtfs.stops b on st_dwithin(a.the_geom, b.the_geom, 0.0045)
        ...
        order by st_distance(a.the_geom, b.the_geom) asc limit 1""".format(origin_stop, direction_id, route.zfill(3))
    res = conn.execute(query)
    return res.fetchone()[0]
